...
SELECT
    ...
    (dul.power_consumption - stats.avg_power) / NULLIF(stats.std_power, 0) AS power_z_score

FROM device_usage_log AS dul

JOIN devices AS d ON dul.device_id = d.device_id

JOIN environmental_data AS ed
    ON d.room_id = ed.room_id
    AND DATE(dul.timestamp) = DATE(ed.timestamp)
    AND HOUR(dul.timestamp) = HOUR(ed.timestamp)

JOIN occupancy_log AS ol
    ON d.room_id = ol.room_id
    AND DATE(dul.timestamp) = DATE(ol.timestamp)
    AND HOUR(dul.timestamp) = HOUR(ol.timestamp)


JOIN (
    SELECT device_id,
           AVG(power_consumption) AS avg_power,
           STDDEV_SAMP(power_consumption) AS std_power
    FROM device_usage_log
    GROUP BY device_id
) AS stats
    ON dul.device_id = stats.device_id
...
# df['power_z_score'] = df.groupby('device_id')['power_consumption'].transform(lambda x: (x - x.mean()) / x.std(ddof=0))
...
print(df[['timestamp', 'device_id', 'power_consumption', 'anomaly']])
print([i for i in df if df['anomaly']==1.any()])
